<div class="col-3 text-bg-info bg-gradient rounded-4">
    @if (Request::is('arrivals/*/edit'))
        <div class="card-body">
            ...
            <form action="{{ route('closeArrival') }}" method="post">
                @csrf
                <div class="mb-3">
                    <input type="hidden" name="arrival_id" value="{{ $editArrival->arrival_id }}">
    ...
    @elseif (Request::is('products', 'brands', 'catalogs', 'shops', 'taxes', 'supliers'))
    ...
    @elseif (Request::is('sales') || Request::is('sales/history'))
        <div class="mb-3">
            ...
            @foreach($Cashes as $Cash)
                @if($Cash->payment_status == 1)
                    <h5>Cash</h5>
                    {{ $Cash->total_calculated_sum * -1}}
                @elseif($Cash->payment_status == 2)
                    <h5>Card</h5>
                    {{ $Cash->total_calculated_sum * -1}}
                @elseif($Cash->payment_status == 3)
                    <h5>Transfer</h5>
                    {{ $Cash->total_calculated_sum * -1}}
                @elseif($Cash->payment_status == 4)
                    <h5>Online</h5>
                    {{ $Cash->total_calculated_sum * -1}}
                @endif
            @endforeach
        ...
        <div class="mb-3">
            ...
            @if($Sale->sale_status == 'PENDING')
                <form action="{{ route('sales.update', $Sale->sale_id) }}" method="post">
                    @method('PATCH')
                    @csrf
                    <label for="sale_status">Payment status</label>
